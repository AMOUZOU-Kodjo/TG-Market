    ...
    <div className="flex items-start justify-between">
      {steps.map((step, i) => {
        ...
        return (
          ...
          >
            {i > 0 && (
              <div className={`absolute top-5 right-1/2 h-0.5 w-full -translate-y-1/2 ${isCompleted || steps[i - 1].status === "completed" ? "bg-brand-500" : "bg-gray-200 dark:bg-gray-700"}`} />
            )}
            ...
            <p className={`mt-2 text-xs font-medium text-center ${isCompleted ? "text-brand-600 dark:text-brand-400" : isCurrent ? "text-yellow-600 dark:text-yellow-400" : "text-gray-400"}`}>
              {step.title}
            </p>
